...
### Structure
```
Xi Manager (Go app) ←→ Redis (cache/sessions)
         ↓
   PostgreSQL (data)
         ↓
...
- **program.go** — entry point and application initialization
- **sources/telegram/** — Telegram Bot API and command handling
- **sources/artificial/** — AI provider orchestration and dialer
- **sources/persistence/** — PostgreSQL and Redis operations
- **sources/repository/** — repositories for data entities
- **sources/texting/** — text processing and formatting
- **sources/throttler/** — rate limiting and spam protection
- **sources/tracing/** — operation logging and tracing

### Module System (Uber-FX)

Each module encapsulates its functionality:
- `ArtificialModule` — AI providers and request processing
- `TelegramModule` — Telegram Bot API and commands
- `PersistenceModule` — database and caching
- `RepositoryModule` — data access
- `ThrottlerModule` — request rate control
- `TracingModule` — logging and monitoring

## Implementation Features

### AI Providers
- **Multiple sources** — support for 5+ different AI APIs
- **Load balancing** — automatic switching between providers
- **Voice processing** — transcription via Whisper API
- **Computer vision** — image analysis via Vision API

### Permissions System
- **Granular permissions** — precise access control to functions
- **Permission hierarchy** — permission inheritance for management convenience
- **Dynamic management** — changing permissions without restart
